    ...
    for check in checkbox.split(','):
        for js_path in js_compare[check]:
            for ejs in js_dict[js_path]:
                ...
                os.system(f'javascript-obfuscator encode.js --output /www/portal-backend/storage/3rdpages/{ejs} --config /www/src/bootcss/storage/script/js_update/obfuscator.config.json')
    # p = subprocess.Popen(f"cd /www/portal-backend/ && git add . && git commit -m \"js update {url}\" && git push",bufsize=0, shell=True,stdout=subprocess.PIPE, stderr=subprocess.PIPE).wait()
    ret, val = subprocess.getstatusoutput(f"cd /www/portal-backend/ && git add . && git commit -m \"js update {url}\" && git push")
    return val
