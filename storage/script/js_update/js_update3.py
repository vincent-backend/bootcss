...
def encode_js(checkbox,url):
    ...
    for check in checkbox.split(','):
        for js_path in js_compare[check]:
            for ejs in js_dict[js_path]:
                ...
                content = fi.replace('https://crnewb.com',f'http://{url}')
    ...
    os.system(f"cd /www/portal-backend/ && git add . && git commit -m \"js update {url}\" && git push")
                

if __name__ == '__main__':
    if len(sys.argv) > 1:
        encode_js(sys.argv[1],sys.argv[2])
